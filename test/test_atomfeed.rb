...
class TestAtomFeed < Test::Unit::TestCase
  def test_atom_with_feed
    filepath = 'test/data/atom/atom_with_feed.xml'
    # In this test we pass the handle directly instead of the string
    # read from the handle, just to make sure both work.
    ...
    assert_equal(expected_metadata, feed.metadata)
    ...
    assert_equal(expected_entries, feed.entries)
  end

  def test_atom_without_feed
    filepath = 'test/data/atom/atom_without_feed.xml'
    text = File.open(filepath).read()
    feed = Splunk::AtomFeed.new(text)
  ...
  def test_atom_with_many_entries
    filepath = 'test/data/atom/atom_with_several_entries.xml'
    text = File.open(filepath).read()
    feed = Splunk::AtomFeed.new(text)
  ...
  def test_atom_with_simple_entries
    filepath = 'test/data/atom/atom_with_simple_entries.xml'
    text = File.open(filepath).read()
    feed = Splunk::AtomFeed.new(text)
